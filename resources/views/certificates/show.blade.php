...
<body class="bg-gray-100 p-4 md:p-10">
    @php
        ...
        $logo = $template?->logo_image
            ? asset('storage/' . $template->logo_image)
            : app(\App\Services\SettingsService::class)->getLogo();
        $background = $template?->background_image ? asset('storage/' . $template->background_image) : null;
        $signature = $template?->signature_image ? asset('storage/' . $template->signature_image) : null;
    @endphp
    ...
    <main class="certificate relative mx-auto aspect-[1.414/1] max-w-5xl overflow-hidden border-[14px] border-green-800 bg-white p-8 text-center shadow-2xl md:p-14">
        ...
        <div class="relative z-10 flex h-full flex-col items-center justify-center">
            <img src="{{ $logo }}" alt="Dhaka IT Institute" class="h-14 w-auto md:h-20">
            ...
            @if($certificate->grade)
                <p class="mt-3 text-lg font-semibold text-gray-700">Grade: {{ $certificate->grade }}</p>
            @endif
            ...
            @if($signature)
                <img src="{{ $signature }}" alt="Signature" class="mt-6 h-12 w-auto md:h-16">
            @endif
